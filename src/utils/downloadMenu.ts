import type { CatalogData } from '../lib/square/types';

export const downloadMenu = (catalog: CatalogData) => {
...
${catalog.dishes.map(d => `<div class="item"><div><div class="item-name">${d.name}</div><div class="item-desc">${d.description}</div></div></div>`).join('')}
</div>
<div class="section"><h3>Catering Packages</h3>
${catalog.packages.map(p => `<div class="pkg"><div class="pkg-title">${p.name}</div><div class="pkg-price">Starting at $${(p.pricePerPersonCents / 100).toFixed(0)}/person · Min ${p.minGuests} guests</div><div class="item-desc" style="margin-top:8px">${p.includes.join(' · ')}</div></div>`).join('')}
</div>
<div class="section"><h3>Add-Ons</h3>
${catalog.addons.map(a => `<div class="item"><span class="item-name">${a.name}</span><span>$${(a.priceCents / 100).toFixed(0)}${a.pricingType === 'per-person' ? '/person' : ' flat'}</span></div>`).join('')}
</div>
